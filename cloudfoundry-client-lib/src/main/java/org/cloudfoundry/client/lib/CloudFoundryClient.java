...
public class CloudFoundryClient implements CloudFoundryOperations {
	...
	public CloudInfo.CC_GENERATION getCloudControllerGeneration() {
		int version = 0;
		try {
			version = Integer.valueOf(getCloudInfo().getVersion());
		} catch (NumberFormatException ignore) {}
		if (version < 2) {
			return CloudInfo.CC_GENERATION.V1;
		}
		return CloudInfo.CC_GENERATION.V2;
	}
