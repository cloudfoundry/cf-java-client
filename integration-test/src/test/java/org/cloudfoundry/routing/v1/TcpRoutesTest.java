...
public final class TcpRoutesTest extends AbstractIntegrationTest {
    ...
    public void events() {
        ...
        Flux.firstWithSignal(
                        ...
                        this.routingClient
                                .tcpRoutes()
                                .events(EventsRequest.builder().build())
                                .filter(
                                        event ->
                                                event.getBackendIp().equals(backendIp)
                                                        && event.getBackendPort()
                                                                .equals(backendPort)
                                                        && event.getPort().equals(port))
                                .take(1))
                .as(StepVerifier::create)
                .expectNextCount(1)
                .expectComplete()
                ...
                .flatMap(
                        routerGroupId ->
                                createTcpRoute(
                                        ...
                                        port,
                                        routerGroupId))
                .then(this.routingClient.tcpRoutes().list(ListTcpRoutesRequest.builder().build()))
                .flatMapIterable(ListTcpRoutesResponse::getTcpRoutes)
                .filter(route -> backendIp.equals(route.getBackendIp()))
                .map(TcpRoute::getPort)
                .as(StepVerifier::create)
    ...
    private static Mono<CreateTcpRoutesResponse> createTcpRoute(
