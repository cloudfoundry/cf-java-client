...
public final class ServiceUsageEventsTest extends AbstractIntegrationTest {
    ...
    public void get() {
        Mono.zip(this.serviceBrokerId, this.spaceId)
                ...
                .flatMap(
                        resource ->
                                Mono.zip(
                                        Mono.just(resource).map(ResourceUtils::getId),
                                        this.cloudFoundryClient
                                                .serviceUsageEvents()
                                                .get(
    ...
    public void listNoneFound() {
        this.cloudFoundryClient
                .serviceUsageEvents()
                .list(ListServiceUsageEventsRequest.builder().serviceId("test-service-id").build())
                .flatMapMany(ResourceUtils::getResources)
                .as(StepVerifier::create)
                .expectComplete()
                .verify(Duration.ofMinutes(5));
    ...
    private static Mono<ServiceUsageEventResource> getFirstEvent(
            CloudFoundryClient cloudFoundryClient) {
        return listServiceUsageEvents(cloudFoundryClient).next();
    }

    private static Mono<ServiceUsageEventResource> getFirstEventWithServiceId(
            CloudFoundryClient cloudFoundryClient) {
    ...
    private static Mono<ServiceUsageEventResource> getSecondEvent(
            CloudFoundryClient cloudFoundryClient) {
        return listServiceUsageEvents(cloudFoundryClient).skip(1).next();
    }

    private static Flux<ServiceUsageEventResource> listServiceUsageEvents(
            CloudFoundryClient cloudFoundryClient) {
        return cloudFoundryClient
                .serviceUsageEvents()
                .list(ListServiceUsageEventsRequest.builder().build())
                .flatMapMany(ResourceUtils::getResources);
    }
    ...
    private static Mono<Void> seedEvents(
            CloudFoundryClient cloudFoundryClient,
