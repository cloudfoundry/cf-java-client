...
public final class ServiceInstancesTest extends AbstractIntegrationTest {
    ...
    public void bindRoute() {
        ...
        Mono.zip(this.organizationId, this.spaceId)
                ...
                .flatMap(
                        function(
                                (domainId, serviceInstanceId, spaceId) ->
                                        Mono.zip(
                                                createRouteId(
                                                        this.cloudFoundryClient,
                                                        domainId,
                ...
                .flatMapMany(
                        serviceInstanceId ->
                                requestListRoutes(this.cloudFoundryClient, serviceInstanceId)
                                        .filter(
                                                route ->
                ...
                .flatMap(
                        job ->
                                JobUtils.waitForCompletion(
                                        ...
                                        (JobEntity) ResourceUtils.getEntity(job)))
                .thenMany(requestListServiceInstances(this.cloudFoundryClient, serviceInstanceName))
                .as(StepVerifier::create)
                .expectComplete()
                .verify(Duration.ofMinutes(5));
    ...
    public void deleteAcceptsIncomplete() {
        ...
        this.spaceId
                ...
                .flatMap(
                        response ->
                                LastOperationUtils.waitForCompletion(
                                        Duration.ofMinutes(5),
                                        () ->
                                                this.cloudFoundryClient
                                                        .serviceInstances()
    ...
    public void deletePurge() {
        ...
        Mono.zip(this.organizationId, this.spaceId)
                ...
                .flatMap(
                        function(
                                (organizationId, serviceInstanceId, spaceId) ->
                                        createAndBindRoute(
                                                        this.cloudFoundryClient,
                                                        domainName,
                                                        organizationId,
                ...
                .flatMap(
                        job ->
                                JobUtils.waitForCompletion(
                                        ...
                                        (JobEntity) ResourceUtils.getEntity(job)))
                .thenMany(
                        requestListServiceInstances(this.cloudFoundryClient, serviceInstanceName)
                                .filter(
                                        resource ->
                ...
                .consumeErrorWith(
                        t ->
                                assertThat(t)
                                        ...
                                        .hasMessageMatching(
                                                "CF-ServiceInstanceNotFound\\([0-9]+\\): The"
    ...
    public void listRoutes() {
        ...
        Mono.zip(this.organizationId, this.spaceId)
                ...
                .flatMap(
                        function(
                                (routeId, serviceInstanceId) ->
                                        requestBindServiceInstanceRoute(
                                                        this.cloudFoundryClient,
                                                        routeId,
                                                        serviceInstanceId)
                                                .thenReturn(serviceInstanceId)))
    ...
    public void listRoutesFilterByDomainId() {
        ...
        Mono.zip(this.organizationId, this.spaceId)
                ...
                .flatMap(
                        function(
                                (domainId, routeId, serviceInstanceId) ->
                                        requestBindServiceInstanceRoute(
                                                        this.cloudFoundryClient,
                                                        routeId,
                                                        serviceInstanceId)
                                                .thenReturn(
                                                        Tuples.of(domainId, serviceInstanceId))))
                .flatMapMany(
                        function(
                                (domainId, serviceInstanceId) ->
                                        Mono.zip(
                                                Mono.just(serviceInstanceId),
                                                PaginationUtils.requestClientV2Resources(
                                                                page ->
        ...
        this.spaceId
                ...
                .thenMany(
                        requestListServiceInstances(
                                this.cloudFoundryClient, newServiceInstanceName))
                .as(StepVerifier::create)
                .expectNextCount(1)
                .expectComplete()
                .verify(Duration.ofMinutes(5));
    ...
    public void upgrade() {
        ...
        this.spaceId
                ...
                .map(
                        resource ->
                                ResourceUtils.getEntity(resource).getMaintenanceInfo().getVersion())
                .as(StepVerifier::create)
                .consumeErrorWith(
                        t ->
                                assertThat(t)
                                        .isInstanceOf(ClientV2Exception.class)
                                        .hasMessageMatching(
                                                "CF-MaintenanceInfoNotSupported\\([0-9]+\\): The"
                                                    + " service broker does not support upgrades"
                                                    + " for service instances created from this"
                                                    + " plan."))
                .verify(Duration.ofMinutes(5));
    }

    private static Mono<BindServiceInstanceRouteResponse> createAndBindRoute(
        ...
        return createPrivateDomainId(cloudFoundryClient, domainName, organizationId)
                .flatMap(
                        domainId ->
                                createRouteId(cloudFoundryClient, domainId, null, null, spaceId))
                .flatMap(
                        routeId ->
                                requestBindServiceInstanceRoute(
                                        cloudFoundryClient, routeId, serviceInstanceId));
    }
    ...
    private static Mono<String> getPlanId(
        ...
        return requestListServices(cloudFoundryClient, serviceBrokerId, serviceName)
                .filter(
                        resource ->
                                serviceName.equals(ResourceUtils.getEntity(resource).getLabel()))
                .single()
                .map(ResourceUtils::getId)
    ...
    private static Mono<BindServiceInstanceRouteResponse> requestBindServiceInstanceRoute(
    ...
    private static Flux<ServiceInstanceResource> requestListServiceInstances(
