...
public final class ServicesTest extends AbstractIntegrationTest {
        ...
        getServiceId(this.cloudFoundryClient, serviceName)
                .flatMapMany(
                        serviceId ->
                                deleteServicePlans(
                                                this.cloudFoundryClient,
                                                serviceBrokerMetadata.serviceBrokerId,
                                                serviceId)
                ...
                .thenMany(
                        requestListServices(
                                this.cloudFoundryClient, serviceBrokerMetadata.serviceBrokerId))
                .filter(
                        resource ->
    ...
    private static Flux<DeleteServicePlanResponse> deleteServicePlans(
            CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceId) {
        return listServicePlanIds(cloudFoundryClient, serviceBrokerId, serviceId)
                .flatMap(
                        servicePlanId ->
                                requestDeleteServicePlan(cloudFoundryClient, servicePlanId));
    }
    ...
    private static Flux<String> listServicePlanIds(
            CloudFoundryClient cloudFoundryClient, String serviceBrokerId, String serviceId) {
        return PaginationUtils.requestClientV2Resources(
                        page ->
                                cloudFoundryClient
    ...
    private static Mono<DeleteServicePlanResponse> requestDeleteServicePlan(
            CloudFoundryClient cloudFoundryClient, String servicePlanId) {
        return cloudFoundryClient
                .servicePlans()
                .delete(DeleteServicePlanRequest.builder().servicePlanId(servicePlanId).build());
    }
    ...
    private Mono<String> createServiceInstanceId(
            CloudFoundryClient cloudFoundryClient, String name, String serviceId, String spaceId) {
                ...
                .flatMap(
                        servicePlanId ->
                                requestCreateServiceInstance(
                                                cloudFoundryClient, name, servicePlanId, spaceId)
                                        .map(ResourceUtils::getId));
    }
}
