...
public final class AuditEventsTest extends AbstractIntegrationTest {
    ...
    // Note: Basic list() test is omitted as the potential volume of data means it take several
    // minutes, with little benefit over the listFilterBy... tests.
    ...
    private static Flux<String> getEventId(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return requestListEvents(cloudFoundryClient, organizationId).map(AuditEventResource::getId);
    }
    ...
    private static Flux<AuditEventResource> requestListEvents(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
