...
/**
 * Abstract goal that provides common configuration for the Cloud Foundry Maven Plugin.
 *
...
public abstract class AbstractCloudFoundryMojo extends AbstractMojo {
    ...
    /**
     * Base execute method. Will perform the login and logout into Cloud Foundry. Delegates to doExecute() for the
    ...
    /**
     * Maven allows for externalizing the credential for server connections to be externalized into settings.xml.
     *
     * If a property was provided on the command line, use that property. Otherwise use the property that was injected
     * via Maven. If that is Null as well, default to the value specified in {@link DefaultConstants}
     *
    ...
    /**
     * If the target property was provided via the command line, use that property. Otherwise use the property that was
     * injected via Maven. If that is Null as well, Null is returned.
     *
        ...
        try {
            ...
            if (HttpStatus.FORBIDDEN.equals(e.getStatusCode())) {
                throw new MojoExecutionException(
                        String.format("Login failed to '%s' using username '%s'. Please verify your login credentials" +
                                ".", target, username), e);
            } else if (HttpStatus.NOT_FOUND.equals(e.getStatusCode())) {
                throw new MojoExecutionException(
                        String.format("The target host '%s' exists but it does not appear to be a valid Cloud Foundry" +
                                " target url.", target), e);
            } else {
                throw e;
            }
     ...
     */
    protected CloudFoundryClient createCloudFoundryClient(String username, String password, URI target, String org,
     ...
     */
    protected CloudFoundryClient createCloudFoundryClient(OAuth2AccessToken token, URI target, String org, String
            space, boolean trustSelfSignedCerts)
            throws MojoExecutionException {
    ...
    private CloudFoundryClient createConnection(CloudCredentials credentials, URI target, String org, String space,
        ...
        try {
            ...
            throw new MojoExecutionException(
                    String.format("Incorrect Cloud Foundry target URL '%s'. Make sure the URL contains a scheme, e.g." +
                            " http://...", target), e);
        }
    }
