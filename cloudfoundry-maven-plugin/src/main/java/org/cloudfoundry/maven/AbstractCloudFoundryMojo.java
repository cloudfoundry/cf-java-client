...
public abstract class AbstractCloudFoundryMojo extends AbstractMojo {
	...
	/**
	 * @parameter expression="${cf.orgname}"
	 */
	private String orgname;

	/**
	 * @parameter expression="${cf.spacename}"
	 */
	 ...
	 * @throws MojoExecutionException
	 */
	protected CloudFoundryClient createCloudFoundryClient(String username, String password, URI target, String orgName, String spaceName)
			throws MojoExecutionException {

		Assert.configurationNotNull(username, "username", SystemProperties.USERNAME);
		Assert.configurationNotNull(password, "password", SystemProperties.PASSWORD);
		Assert.configurationNotNull(target, "target", SystemProperties.TARGET);
		Assert.configurationNotNull(orgName, "orgname", SystemProperties.ORGNAME);
		Assert.configurationNotNull(spaceName, "spacename", SystemProperties.SPACENAME);
		...
		try {
			...
			for (CloudSpace space : authClient.getSpaces()) {
				if (space.getName().equals(spaceName) && space.getOrganization().getName().equals(orgName)) {
					client = new CloudFoundryClient(new CloudCredentials(username, password), target.toURL(), space);
					return client;
				}
			}
		} catch (MalformedURLException e) {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			if (simpleClient.getCloudInfo().getCloudControllerMajorVersion() == CloudInfo.CC_MAJOR_VERSION.V2) {
				client = createCloudFoundryClient(getUsername(), getPassword(), getTarget(), getOrgName(), getSpaceName());
				if (client != null) {
					connectToCloudFoundry(client);
				} else {
					throw new MojoExecutionException(
							String.format("Incorrect Cloud Foundry space or org, are you sure '%s' and '%s' is correct?", getSpaceName(), getOrgName()));
				}
			} else {
	...
	public String getOrgName() {
		Assert.notNull(orgname, "The org is not set.");
		return orgname;
	}

	public String getSpaceName() {
		Assert.notNull(spacename, "The space is not set.");
		return spacename;
	}
}
