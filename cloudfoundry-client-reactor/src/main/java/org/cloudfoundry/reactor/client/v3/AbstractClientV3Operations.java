...
public abstract class AbstractClientV3Operations extends AbstractReactorOperations {
    ...
    protected final Mono<HttpClientResponse> get(Object request, Function<UriComponentsBuilder, UriComponentsBuilder> uriTransformer) {
        return doGet(getUriAugmenter(request, uriTransformer), outbound -> outbound.failOnClientError(false), ClientErrorMapper.cloudFoundry(this.connectionContext.getObjectMapper()));
    }
    ...
    protected final <T> Mono<T> patch(Object request, Class<T> responseType, Function<UriComponentsBuilder, UriComponentsBuilder> uriTransformer) {
        return doPatch(request, responseType, getUriAugmenter(request, uriTransformer), outbound -> outbound.failOnClientError(false),
            ClientErrorMapper.cloudFoundry(this.connectionContext.getObjectMapper()));
    }
