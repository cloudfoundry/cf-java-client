...
public final class ReactorProcesses extends AbstractClientV3Operations implements Processes {
    ...
    public Mono<GetProcessStatisticsResponse> getStatistics(GetProcessStatisticsRequest request) {
        return get(
                        request,
                        GetProcessStatisticsResponse.class,
                        builder ->
                                builder.pathSegment("processes", request.getProcessId(), "stats"))
                .checkpoint();
    }
    ...
    public Mono<ListProcessesResponse> list(ListProcessesRequest request) {
        return get(
                        request,
                        ListProcessesResponse.class,
                        builder -> builder.pathSegment("processes"))
                .checkpoint();
    }
    ...
    public Mono<ScaleProcessResponse> scale(ScaleProcessRequest request) {
        return post(
                        request,
                        ScaleProcessResponse.class,
                        builder ->
                                builder.pathSegment(
                                        "processes", request.getProcessId(), "actions", "scale"))
                .checkpoint();
    }
