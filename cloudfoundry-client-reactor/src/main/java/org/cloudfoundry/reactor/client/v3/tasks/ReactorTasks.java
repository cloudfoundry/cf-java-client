...
public final class ReactorTasks extends AbstractClientV3Operations implements Tasks {
    ...
    @Override
    public Mono<CancelTaskResponse> cancel(
            CancelTaskRequest request) { // TODO: Modify once support has aged out
        // https://v3-apidocs.cloudfoundry.org/version/3.27.0/#cancel-a-task
        return put(
                        request,
                        CancelTaskResponse.class,
                        builder -> builder.pathSegment("tasks", request.getTaskId(), "cancel"))
                .checkpoint();
    }
    ...
    public Mono<CreateTaskResponse> create(CreateTaskRequest request) {
        return post(
                        ...
                        builder -> builder.pathSegment("apps", request.getApplicationId(), "tasks"))
                .checkpoint();
    }
    ...
    public Mono<GetTaskResponse> get(GetTaskRequest request) {
        return get(
                        ...
                        builder -> builder.pathSegment("tasks", request.getTaskId()))
                .checkpoint();
    }
