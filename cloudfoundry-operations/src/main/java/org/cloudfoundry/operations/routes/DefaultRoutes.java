...
public final class DefaultRoutes implements Routes {
    ...
    public Mono<Integer> create(CreateRouteRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.organizationId)
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId, domainId) ->
                                        requestCreateRoute(
                                                        cloudFoundryClient,
                                                        domainId,
                                                        request.getHost(),
    ...
    public Flux<Route> list(ListRoutesRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.organizationId)
                ...
                .flatMapMany(
                        function(
                                (cloudFoundryClient, domains, spaces) ->
                                        getRoutes(
                                                        cloudFoundryClient,
                                                        request,
                                                        this.organizationId,
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, domains, route, spaces) ->
                                        Mono.zip(
                                                getApplicationNames(
                                                        cloudFoundryClient,
    ...
    public Mono<Integer> map(MapRouteRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.organizationId, this.spaceId)
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, routeResource, applicationId) ->
    ...
    private static Mono<Map<String, String>> getAllDomains(
        ...
        return requestAllPrivateDomains(cloudFoundryClient, organizationId)
                ...
                .mergeWith(
                        requestAllSharedDomains(cloudFoundryClient)
                                .map(
                                        resource ->
                                                Tuples.of(
                                                        ResourceUtils.getId(resource),
    ...
    private static Mono<Map<String, String>> getAllSpaces(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
    ...
    private static Flux<Resource<?>> getDomains(
            CloudFoundryClient cloudFoundryClient, String organizationId, String domain) {
    ...
    private static Mono<AbstractRouteResource> getOrCreateRoute(
        ...
        return getDomainId(cloudFoundryClient, organizationId, domain)
                .flatMap(
                        domainId ->
                                getRoute(cloudFoundryClient, domainId, host, path, port)
    ...
    private static Mono<RouteResource> getRoute(
        ...
        return getRoute(cloudFoundryClient, domainId, host, path, port)
                .switchIfEmpty(
                        ExceptionUtils.illegalArgument("Route for %s does not exist", domain));
    }
    ...
    private static Flux<RouteResource> getRoutes(
            CloudFoundryClient cloudFoundryClient,
            ListRoutesRequest request,
            Mono<String> organizationId,
            Mono<String> spaceId) {
        if (Level.ORGANIZATION == request.getLevel()) {
            return organizationId.flatMapMany(
                    organizationId1 ->
                            requestRoutes(
                                    cloudFoundryClient,
                                    builder -> builder.organizationId(organizationId1)));
        } else {
            return spaceId.flatMapMany(
                    spaceId1 -> requestSpaceRoutes(cloudFoundryClient, spaceId1));
        }
    }
    ...
    private static Mono<Optional<String>> getServiceName(
            CloudFoundryClient cloudFoundryClient, RouteEntity route) {
        return Mono.justOrEmpty(route.getServiceInstanceId())
                .flatMap(
                        serviceInstanceId ->
                                getServiceInstanceName(
                                        cloudFoundryClient, serviceInstanceId, route.getSpaceId()))
    ...
    private static Flux<SpaceResource> requestAllSpaces(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Flux<RouteResource> requestRoutes(
        ...
        org.cloudfoundry.client.v2.routes.ListRoutesRequest.Builder listBuilder =
                modifier.apply(org.cloudfoundry.client.v2.routes.ListRoutesRequest.builder());
    ...
    private static Flux<RouteResource> requestSpaceRoutes(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
