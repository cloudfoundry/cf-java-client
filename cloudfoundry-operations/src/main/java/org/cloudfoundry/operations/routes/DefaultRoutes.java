...
public final class DefaultRoutes implements Routes {
        ...
        public DefaultRoutes(Mono<CloudFoundryClient> cloudFoundryClient,
        ...
        public Mono<Boolean> check(CheckRouteRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.organizationId)
                                .flatMap(function((client, organizationId) -> Mono.zip(
                                                this.cloudFoundryClient,
                                                getOptionalDomainIdByName(client, organizationId,
                                                                request.getDomain()))))
                                .flatMap(function((client, domainId) -> routeExists(client,
                                                domainId, request.getHost(), request.getPath())))
                                .defaultIfEmpty(false)
                                .transform(OperationsLogging.log("Check Route Exists"))
                                .checkpoint();
        }

        private static Mono<Boolean> routeExists(CloudFoundryClient cloudFoundryClient,
                        String domainId, String host, String path) {
                return cloudFoundryClient.domainsV3()
                                .checkReservedRoutes(CheckReservedRoutesRequest.builder()
                                                .domainId(domainId).host(host).path(path).build())
                                .flatMap(response -> Mono.just(response.getMatchingRoute()));
        }

        private static Mono<String> getOptionalDomainIdByName(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String domain) {
                return listDomains(cloudFoundryClient, organizationId, new String[] {domain})
                                .singleOrEmpty().map(resource -> resource.getId());
        }
        ...
        private static Flux<DomainResource> listDomains(CloudFoundryClient cloudFoundryClient,
                ...
                return PaginationUtils.requestClientV3Resources(page -> cloudFoundryClient
                                .organizationsV3()
                                .listDomains(ListOrganizationDomainsRequest.builder()
                                                .names(domainNamesFilter).page(page)
        ...
        public Mono<Integer> create(CreateRouteRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.organizationId)
                                .flatMap(function((cloudFoundryClient, organizationId) -> Mono.zip(
                                                ...
                                                getSpaceId(cloudFoundryClient, organizationId,
                                                                request.getSpace()),
                                                getDomainId(cloudFoundryClient, organizationId,
                                                                request.getDomain()))))
                                .flatMap(function((cloudFoundryClient, spaceId,
                                                domainId) -> requestCreateRoute(cloudFoundryClient,
                                                                domainId, request.getHost(),
                                                                request.getPath(),
                                                                request.getPort(), request
                                                                                .getRandomPort(),
                                                                spaceId).map(ResourceUtils::getEntity)
                                                                                .flatMap(routeEntity -> Mono
                                                                                                .justOrEmpty(routeEntity
                                                                                                                .getPort()))))
        ...
        public Mono<Void> delete(DeleteRouteRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.organizationId)
                                ...
                                .flatMap(function((cloudFoundryClient, domainId) -> Mono.zip(
                                                ...
                                                getRouteId(cloudFoundryClient, request.getHost(),
                                                                request.getDomain(), domainId,
                                                                request.getPath(),
                                                                request.getPort()))))
                                .flatMap(function(DefaultRoutes::deleteRoute))
                                .transform(OperationsLogging.log("Delete Route")).checkpoint();
        }

        @Override
        public Mono<Void> deleteOrphanedRoutes(DeleteOrphanedRoutesRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.spaceId).flatMap(function((client,
                                spaceId) -> Mono.zip(this.cloudFoundryClient, client.spacesV3()
                                                .deleteUnmappedRoutes(DeleteUnmappedRoutesRequest
                                                                .builder().spaceId(spaceId)
                                                                .build()))))
                                .flatMap(function((client, job) -> JobUtils.waitForCompletion(
        ...
        public Flux<Route> list(ListRoutesRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.organizationId)
                                ...
                                .flatMapMany(function((cloudFoundryClient, domains,
                                                spaces) -> getRoutes(cloudFoundryClient, request,
                                                                this.organizationId, this.spaceId)
                                                                                .map(route -> Tuples
                                                                                                .of(cloudFoundryClient,
                                                                                                                domains,
                                                                                                                route,
                                                                                                                spaces))))
                                .flatMap(function((cloudFoundryClient, domains, route,
                                                spaces) -> Mono.zip(getApplicationNames(
                                                                cloudFoundryClient,
                                                                ResourceUtils.getId(route)),
                                                                getDomainName(domains, ResourceUtils
                                                                                .getEntity(route)
                                                                                .getDomainId()),
                                                                Mono.just(route),
                                                                getServiceName(cloudFoundryClient,
                                                                                ResourceUtils.getEntity(
                                                                                                route)),
                                                                getSpaceName(spaces, ResourceUtils
                                                                                .getEntity(route)
                                                                                .getSpaceId()))))
                                .map(function(DefaultRoutes::toRoute))
                                .transform(OperationsLogging.log("List Routes")).checkpoint();
        }

        @Override
        public Mono<Integer> map(MapRouteRequest request) {
                return Mono.zip(this.cloudFoundryClient, this.organizationId, this.spaceId).flatMap(
                                function((cloudFoundryClient, organizationId, spaceId) -> Mono.zip(
                                                Mono.just(cloudFoundryClient),
                                                getOrCreateRoute(cloudFoundryClient, organizationId,
                                                                spaceId, request.getDomain(),
                                                                request.getHost(),
                                                                request.getPath(),
                                                                request.getPort(),
                                                                request.getRandomPort()),
                                                getApplicationId(cloudFoundryClient,
        ...
        private static Mono<Map<String, String>> getAllDomains(
                ...
                return requestAllPrivateDomains(cloudFoundryClient, organizationId)
                                ...
                                .mergeWith(requestAllSharedDomains(cloudFoundryClient).map(
                                                resource -> Tuples.of(ResourceUtils.getId(resource),
                                                                ResourceUtils.getEntity(resource)
                                                                                .getName())))
                                .collectMap(function((id, name) -> id),
        ...
        private static Mono<ApplicationResource> getApplication(
                ...
                return requestApplications(cloudFoundryClient, application, spaceId).single()
        ...
        private static Mono<String> getApplicationId(CloudFoundryClient cloudFoundryClient,
                        String application, String spaceId) {
                return getApplication(cloudFoundryClient, application, spaceId)
                                .map(ResourceUtils::getId);
        }

        private static Mono<List<String>> getApplicationNames(CloudFoundryClient cloudFoundryClient,
                        String routeId) {
        ...
        private static Mono<Resource<?>> getDomain(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String domain) {
                return getDomains(cloudFoundryClient, organizationId, domain).single()
                                .onErrorResume(NoSuchElementException.class,
                                                t -> ExceptionUtils.illegalArgument(
        ...
        private static Mono<String> getDomainId(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String domain) {
        ...
        private static Flux<Resource<?>> getDomains(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String domain) {
        ...
        private static Mono<AbstractRouteResource> getOrCreateRoute(
                        CloudFoundryClient cloudFoundryClient, String organizationId,
                        String spaceId, String domain, String host, String path, Integer port,
                        Boolean randomPort) {
                if (randomPort != null) {
                        return getDomainId(cloudFoundryClient, organizationId, domain).flatMap(
                                        domainId -> requestCreateRoute(cloudFoundryClient, domainId,
                                                        host, path, port, randomPort, spaceId));
                }

                return getDomainId(cloudFoundryClient, organizationId, domain).flatMap(
                                domainId -> getRoute(cloudFoundryClient, domainId, host, path, port)
                                                .cast(AbstractRouteResource.class)
                                                .switchIfEmpty(requestCreateRoute(
                                                                cloudFoundryClient, domainId, host,
                                                                path, port, randomPort, spaceId)));
        }

        private static Mono<RouteResource> getRoute(CloudFoundryClient cloudFoundryClient,
                        String domainId, String domain, String host, String path, Integer port) {
                return getRoute(cloudFoundryClient, domainId, host, path, port).switchIfEmpty(
                                ExceptionUtils.illegalArgument("Route for %s does not exist",
                                                domain));
        }

        private static Mono<RouteResource> getRoute(CloudFoundryClient cloudFoundryClient,
                        String domainId, String host, String path, Integer port) {
                if (port != null) {
        ...
        private static Mono<String> getRouteId(CloudFoundryClient cloudFoundryClient, String host,
                        String domain, String domainId, String path, Integer port) {
                return getRoute(cloudFoundryClient, domainId, domain, host, path, port)
                                .map(ResourceUtils::getId);
        }

        private static Flux<RouteResource> getRoutes(CloudFoundryClient cloudFoundryClient,
                        ListRoutesRequest request, Mono<String> organizationId,
                        Mono<String> spaceId) {
                if (Level.ORGANIZATION == request.getLevel()) {
                        return organizationId.flatMapMany(organizationId1 -> requestRoutes(
                                        cloudFoundryClient,
                                        builder -> builder.organizationId(organizationId1)));
                } else {
                        return spaceId.flatMapMany(spaceId1 -> requestSpaceRoutes(
                                        cloudFoundryClient, spaceId1));
                }
        }

        private static Mono<Optional<String>> getServiceInstanceName(
                        CloudFoundryClient cloudFoundryClient, String serviceInstanceId,
                        String spaceId) {
                return requestListSpaceServiceInstances(cloudFoundryClient, spaceId).filter(
                                resource -> serviceInstanceId.equals(ResourceUtils.getId(resource)))
                                .single().map(resource -> Optional
                                                .of(ResourceUtils.getEntity(resource).getName()));
        }

        private static Mono<Optional<String>> getServiceName(CloudFoundryClient cloudFoundryClient,
                        RouteEntity route) {
                return Mono.justOrEmpty(route.getServiceInstanceId())
                                .flatMap(serviceInstanceId -> getServiceInstanceName(
                                                cloudFoundryClient, serviceInstanceId,
                                                route.getSpaceId()))
                                .defaultIfEmpty(Optional.empty());
        }

        private static Mono<SpaceResource> getSpace(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String space) {
                return requestSpaces(cloudFoundryClient, organizationId, space).single()
                                .onErrorResume(NoSuchElementException.class, t -> ExceptionUtils
                                                .illegalArgument("Space %s does not exist", space));
        }

        private static Mono<String> getSpaceId(CloudFoundryClient cloudFoundryClient,
                        String organizationId, String space) {
        ...
        private static Flux<PrivateDomainResource> requestAllPrivateDomains(
                        CloudFoundryClient cloudFoundryClient, String organizationId) {
        ...
        private static Flux<SharedDomainResource> requestAllSharedDomains(
        ...
        private static Flux<SpaceResource> requestAllSpaces(CloudFoundryClient cloudFoundryClient,
                        String organizationId) {
        ...
        private static Flux<ApplicationResource> requestApplications(
                        CloudFoundryClient cloudFoundryClient, String routeId) {
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient.routes()
                                .listApplications(ListRouteApplicationsRequest.builder()
                                                .routeId(routeId).page(page).build()));
        }
        ...
        private static Flux<ApplicationResource> requestApplications(
                ...
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient.spaces()
                                .listApplications(ListSpaceApplicationsRequest.builder()
                                                .name(application).page(page).spaceId(spaceId)
                                                .build()));
        }
        ...
        private static Mono<AssociateApplicationRouteResponse> requestAssociateRoute(
                ...
                return cloudFoundryClient.applicationsV2()
                                .associateRoute(AssociateApplicationRouteRequest.builder()
                                                .applicationId(applicationId).routeId(routeId)
                                                .build());
        }
        ...
        private static Mono<CreateRouteResponse> requestCreateRoute(
                        ...
                        String path, Integer port, Boolean randomPort, String spaceId) {
        ...
        private static Mono<Void> deleteRoute(CloudFoundryClient cloudFoundryClient,
                        Duration completionTimeout, String routeId) {
                return requestDeleteRoute(cloudFoundryClient, routeId).flatMap(job -> JobUtils
                                .waitForCompletion(cloudFoundryClient, completionTimeout, job));
        }

        private static Mono<String> requestDeleteRoute(CloudFoundryClient cloudFoundryClient,
                        String routeId) {
                return cloudFoundryClient.routesV3()
                                .delete(org.cloudfoundry.client.v3.routes.DeleteRouteRequest
                                                .builder().routeId(routeId).build());
        }
        ...
        private static Flux<UnionServiceInstanceResource> requestListSpaceServiceInstances(
                ...
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient.spaces()
                                .listServiceInstances(ListSpaceServiceInstancesRequest.builder()
                                                .page(page).returnUserProvidedServiceInstances(true)
                                                .spaceId(spaceId).build()));
        }

        private static Flux<PrivateDomainResource> requestPrivateDomains(
                        CloudFoundryClient cloudFoundryClient, String organizationId,
                        String domain) {
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient
                                .organizations()
                                .listPrivateDomains(ListOrganizationPrivateDomainsRequest.builder()
                                                .organizationId(organizationId).name(domain)
                                                .page(page).build()));
        }
        ...
        private static Mono<Void> requestRemoveRouteFromApplication(
                        CloudFoundryClient cloudFoundryClient, String applicationId,
                        String routeId) {
                return cloudFoundryClient.applicationsV2().removeRoute(RemoveApplicationRouteRequest
                                .builder().applicationId(applicationId).routeId(routeId).build());
        }
        ...
        private static Flux<RouteResource> requestRoutes(CloudFoundryClient cloudFoundryClient,
                ...
                org.cloudfoundry.client.v2.routes.ListRoutesRequest.Builder listBuilder =
                                modifier.apply(org.cloudfoundry.client.v2.routes.ListRoutesRequest
                                                .builder());

                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient.routes()
                                .list(listBuilder.page(page).build()));
        }

        private static Flux<RouteResource> requestRoutes(CloudFoundryClient cloudFoundryClient,
                        String domainId, String host, String path, Integer port) {
                return requestRoutes(cloudFoundryClient, builder -> builder.domainId(domainId)
                                .hosts(Optional.ofNullable(host).map(Collections::singletonList)
                                                .orElse(null))
                                .paths(Optional.ofNullable(path).map(Collections::singletonList)
                                                .orElse(null))
        ...
        private static Flux<SharedDomainResource> requestSharedDomains(
                ...
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient
                                .sharedDomains().list(ListSharedDomainsRequest.builder()
                                                .name(domain).page(page).build()));
        }

        private static Flux<RouteResource> requestSpaceRoutes(CloudFoundryClient cloudFoundryClient,
                        String spaceId) {
                return PaginationUtils.requestClientV2Resources(page -> cloudFoundryClient.spaces()
                                .listRoutes(ListSpaceRoutesRequest.builder().spaceId(spaceId)
                                                .page(page).build()));
        }
        ...
        private static Flux<SpaceResource> requestSpaces(CloudFoundryClient cloudFoundryClient,
                ...
                return PaginationUtils
                                .requestClientV2Resources(page -> cloudFoundryClient.organizations()
                                                .listSpaces(ListOrganizationSpacesRequest.builder()
                                                                .organizationId(organizationId)
                                                                .name(space).page(page).build()));
        }

        private static Route toRoute(List<String> applications, String domain,
                        RouteResource resource, Optional<String> service, String space) {
                RouteEntity entity = ResourceUtils.getEntity(resource);
                Route.Builder builder = Route.builder().applications(applications).domain(domain)
                                .host(entity.getHost()).id(ResourceUtils.getId(resource))
                                .path(entity.getPath()).space(space);
