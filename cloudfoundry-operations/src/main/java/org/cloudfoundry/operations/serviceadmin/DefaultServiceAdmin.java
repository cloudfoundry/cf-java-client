...
public final class DefaultServiceAdmin implements ServiceAdmin {
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, servicePlans) ->
                                        Mono.when(
                                                updateServicePlanVisibilities(
                                                        cloudFoundryClient, request, servicePlans),
                                                updateServicePlansPublicStatus(
                                                        cloudFoundryClient,
                                                        request,
                                                        servicePlans))))
                .then()
                        ...
                        function(
                                (cloudFoundryClient, serviceBrokerId) ->
                                        requestUpdateServiceBroker(
                                                cloudFoundryClient, request, serviceBrokerId)))
                .then()
                .transform(OperationsLogging.log("Update Service Broker"))
                .checkpoint();
    }

    private static Flux<ServiceAccess> collectServiceAccessSettings(
            CloudFoundryClient cloudFoundryClient,
            List<ServiceBrokerResource> brokers,
            ListServiceAccessSettingsRequest request,
        ...
        if (brokerIds.size() == 0) {
            throw new IllegalArgumentException(
                    String.format("Service Broker %s not found", request.getBrokerName()));
        }
        ...
        return Mono.when(
                ...
                .flatMapMany(
                        services ->
                                getServicePlans(cloudFoundryClient, services)
                ...
                .filter(
                        predicate(
                                (organizationNames, services, servicePlan) ->
        ...
        return requestListOrganizations(cloudFoundryClient, organizationName)
                ...
                .switchIfEmpty(
                        ExceptionUtils.illegalArgument(
                                "Organization %s not found", organizationName));
    }

    private static Mono<List<String>> getOrganizationNames(
            CloudFoundryClient cloudFoundryClient,
                ...
                .filter(
                        visibility ->
                                servicePlan
                                        .getMetadata()
                                        .getId()
                                        .equals(visibility.getEntity().getServicePlanId()))
                .flatMap(
                        visibility ->
                                requestGetOrganization(
                                        cloudFoundryClient,
                                        visibility.getEntity().getOrganizationId()))
                .map(organization -> ResourceUtils.getEntity(organization).getName())
                .collectList();
    }

    private static Mono<ServiceBrokerResource> getServiceBroker(
        ...
        return requestListServiceBrokers(cloudFoundryClient, serviceBrokerName)
                ...
                .onErrorResume(
                        ...
                        t ->
                                ExceptionUtils.illegalArgument(
                                        "Service Broker %s not found", serviceBrokerName));
    }

    private static Mono<String> getServiceBrokerId(
            CloudFoundryClient cloudFoundryClient, String serviceBrokerName) {
    ...
    private static Mono<String> getServiceId(
        ...
        return requestListServices(cloudFoundryClient, serviceName)
                ...
                .switchIfEmpty(
                        ExceptionUtils.illegalArgument(
                                "Service offering %s not found", serviceName));
    }
    ...
    private static Flux<ServicePlanResource> getServicePlans(
            CloudFoundryClient cloudFoundryClient, List<ServiceResource> services) {
        List<String> serviceIds =
                services.stream().map(ResourceUtils::getId).collect(Collectors.toList());
    ...
    private static boolean isUpdateableServicePlan(
            String servicePlanName, ServicePlanResource servicePlan) {
        return servicePlanName == null
                || servicePlanName.isEmpty()
                || servicePlanName.equals(ResourceUtils.getEntity(servicePlan).getName());
    }

    private static boolean isVisibleOrganization(
    ...
    private static boolean isVisibleService(
            ListServiceAccessSettingsRequest request, ServiceResource service) {
        return request.getServiceName() == null
                || request.getServiceName().isEmpty()
                || request.getServiceName().equals(ResourceUtils.getEntity(service).getLabel());
    }

    private static Mono<List<ServiceBrokerResource>> listServiceBrokers(
            CloudFoundryClient cloudFoundryClient) {
    ...
    private static Flux<String> listServicePlanVisibilityIds(
            CloudFoundryClient cloudFoundryClient, List<String> servicePlanIds) {
    ...
    private static Flux<String> listServicePlanVisibilityIds(
        ...
        return requestListServicePlanVisibilities(
                        cloudFoundryClient, organizationId, servicePlanIds)
                .map(ResourceUtils::getId);
    }
        ...
        return cloudFoundryClient
                ...
                .create(
                        org.cloudfoundry.client.v2.servicebrokers.CreateServiceBrokerRequest
                                ...
                                .spaceId(
                                        Optional.ofNullable(isSpaceScoped).orElse(false)
                                                ? spaceId
                                                : null)
                                .build());
    }

    private static Mono<CreateServicePlanVisibilityResponse> requestCreateServicePlanVisibility(
    ...
    private static Mono<Void> requestDeleteServiceBroker(
    ...
    private static Mono<DeleteServicePlanVisibilityResponse> requestDeleteServicePlanVisibility(
            CloudFoundryClient cloudFoundryClient, String visibilityId) {
        return cloudFoundryClient
                .servicePlanVisibilities()
                .delete(
    ...
    private static Flux<ServiceBrokerResource> requestListServiceBrokers(
            CloudFoundryClient cloudFoundryClient) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
                                .serviceBrokers()
                                .list(ListServiceBrokersRequest.builder().page(page).build()));
    }
    ...
    private static Mono<List<ServicePlanVisibilityResource>> requestListServicePlanVisibilities(
            CloudFoundryClient cloudFoundryClient) {
        return PaginationUtils.requestClientV2Resources(
                        page ->
                                cloudFoundryClient
    ...
    private static Flux<ServicePlanResource> requestListServicePlans(
            CloudFoundryClient cloudFoundryClient, List<String> services) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
                                .servicePlans()
    ...
    private static Flux<ServiceResource> requestListServices(
            CloudFoundryClient cloudFoundryClient, List<String> brokerIds) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
                                .services()
    ...
    private static Mono<UpdateServiceBrokerResponse> requestUpdateServiceBroker(
            CloudFoundryClient cloudFoundryClient,
            UpdateServiceBrokerRequest request,
    ...
    private static Mono<UpdateServicePlanResponse> requestUpdateServicePlanPublicStatus(
    ...
    private static ServiceAccess toServiceAccess(
            List<ServiceBrokerResource> brokers,
            List<String> organizationNames,
            List<ServiceResource> services,
            ServicePlanResource servicePlan) {
        ...
        ServiceResource service =
                services.stream()
                        .filter(
                                item ->
                                        ResourceUtils.getId(item)
                                                .equals(servicePlan.getEntity().getServiceId()))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new IllegalStateException(
                                                String.format(
                                                        "Unable to find service for %s",
                                                        servicePlanName)));
        ...
        String brokerName =
                brokers.stream()
                        .filter(
                                broker ->
                                        ResourceUtils.getId(broker)
                                                .equals(
                                                        ResourceUtils.getEntity(service)
                                                                .getServiceBrokerId()))
    ...
    private static Mono<Void> updateServicePlanVisibilities(
            CloudFoundryClient cloudFoundryClient,
            EnableServiceAccessRequest request,
            List<ServicePlanResource> servicePlans) {
        List<String> servicePlanIds =
                servicePlans.stream()
                        .filter(
                                servicePlan ->
                                        isUpdateableServicePlan(
                                                request.getServicePlanName(), servicePlan))
                        .map(ResourceUtils::getId)
                        .collect(Collectors.toList());
        ...
        if (request.getOrganizationName() != null && !request.getOrganizationName().isEmpty()) {
            return getOrganizationId(cloudFoundryClient, request.getOrganizationName())
                    .flatMapMany(
                            organizationId ->
                                    Flux.fromIterable(servicePlanIds)
                                            .flatMap(
                                                    servicePlanId ->
                    ...
                    .flatMap(
                            visibilityId ->
                                    requestDeleteServicePlanVisibility(
                                                    cloudFoundryClient, visibilityId)
                                            .flatMap(
                                                    job ->
    ...
    private static Mono<Void> updateServicePlanVisibilities(
            CloudFoundryClient cloudFoundryClient,
            DisableServiceAccessRequest request,
            List<ServicePlanResource> servicePlans) {
        List<String> servicePlanIds =
                servicePlans.stream()
                        .filter(
    ...
    private static Mono<Void> updateServicePlansPublicStatus(
            CloudFoundryClient cloudFoundryClient,
            DisableServiceAccessRequest request,
            List<ServicePlanResource> servicePlans) {
        if (request.getOrganizationName() != null && !request.getOrganizationName().isEmpty()) {
        ...
        return Flux.fromIterable(servicePlans)
                ...
                .flatMap(
                        servicePlan ->
                                requestUpdateServicePlanPublicStatus(
                                        cloudFoundryClient,
                                        false,
                                        ResourceUtils.getId(servicePlan)))
                .then();
    }
    ...
    private static Mono<Void> updateServicePlansPublicStatus(
        ...
        return Flux.fromIterable(servicePlans)
                .filter(
                        servicePlan ->
                                isUpdateableServicePlan(request.getServicePlanName(), servicePlan))
                .filter(servicePlan -> !ResourceUtils.getEntity(servicePlan).getPubliclyVisible())
                .flatMap(
                        servicePlan ->
                                requestUpdateServicePlanPublicStatus(
                                        cloudFoundryClient, true, ResourceUtils.getId(servicePlan)))
                .then();
    }

    private static Mono<Void> validateOrganization(
            CloudFoundryClient cloudFoundryClient, String organizationName) {
    ...
    private static Mono<Void> validateService(
            CloudFoundryClient cloudFoundryClient, String serviceName) {
        if (serviceName != null) {
            return requestListServices(cloudFoundryClient, serviceName)
                    .switchIfEmpty(
                            ExceptionUtils.illegalArgument("Service %s not found", serviceName))
                    .then();
        } else {
            return Mono.empty();
        }
