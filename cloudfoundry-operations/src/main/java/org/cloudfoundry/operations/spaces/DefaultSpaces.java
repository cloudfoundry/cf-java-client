...
public final class DefaultSpaces implements Spaces {
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId) ->
                                        requestUpdateSpaceSsh(cloudFoundryClient, spaceId, true)))
                .then()
    ...
    public Mono<Void> create(CreateSpaceRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.username)
                .flatMap(
                        function(
                                (cloudFoundryClient, username) ->
                                        Mono.zip(
                                                Mono.just(cloudFoundryClient),
                                                Mono.just(username),
                                                getOrganizationIdOrDefault(
                                                        cloudFoundryClient,
                                                        request.getOrganization(),
                                                        this.organizationId))))
                .flatMap(
                        function(
                                (cloudFoundryClient, username, organizationId) ->
                                        Mono.zip(
                                                Mono.just(cloudFoundryClient),
                                                Mono.just(username),
                                                Mono.just(organizationId),
                                                getOptionalSpaceQuotaId(
                                                        cloudFoundryClient,
                                                        organizationId,
                                                        request.getSpaceQuota()))))
                .flatMap(
                        function(
                                (cloudFoundryClient, username, organizationId, spaceQuotaId) ->
                                        Mono.zip(
                                                Mono.just(cloudFoundryClient),
                                                Mono.just(organizationId),
                                                requestCreateSpace(
                                                                ...
                                                                organizationId,
                                                                request.getName(),
                                                                spaceQuotaId.orElse(null))
                                                        .map(ResourceUtils::getId),
                                                Mono.just(username))))
                .delayUntil(
                        function(
                                (cloudFoundryClient, organizationId, spaceId, username) ->
                                        requestAssociateOrganizationUserByUsername(
                                                cloudFoundryClient, organizationId, username)))
                .flatMap(
                        function(
                                (cloudFoundryClient, organizationId, spaceId, username) ->
                                        Mono.zip(
                                                requestAssociateSpaceManagerByUsername(
                                                        cloudFoundryClient, spaceId, username),
                                                requestAssociateSpaceDeveloperByUsername(
                                                        cloudFoundryClient, spaceId, username))))
                .then()
    ...
    public Mono<Void> disallowSsh(DisallowSpaceSshRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.organizationId)
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId) ->
                                        requestUpdateSpaceSsh(cloudFoundryClient, spaceId, false)))
                .then()
        ...
        return Mono.zip(this.cloudFoundryClient, this.organizationId)
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId) ->
                                        requestUpdateSpace(
                                                cloudFoundryClient, spaceId, request.getNewName())))
                .then()
    ...
    private static Mono<Void> deleteSpace(
            CloudFoundryClient cloudFoundryClient, Duration completionTimeout, String spaceId) {
    ...
    private static Mono<List<String>> getApplicationNames(
            CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    ...
    private static Mono<List<String>> getDomainNames(
            CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
        return requestListPrivateDomains(
                        cloudFoundryClient, spaceResource.getEntity().getOrganizationId())
    ...
    private static Mono<Optional<SpaceQuota>> getOptionalSpaceQuotaDefinition(
            CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
        String spaceQuotaDefinitionId =
                ResourceUtils.getEntity(spaceResource).getSpaceQuotaDefinitionId();
    ...
    private static Mono<Optional<String>> getOptionalSpaceQuotaId(
            CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
        if (spaceQuota == null) {
            return Mono.just(Optional.empty());
        } else {
    ...
    private static Mono<String> getOrganizationIdOrDefault(
    ...
    private static Mono<String> getOrganizationName(
            CloudFoundryClient cloudFoundryClient, SpaceResource resource) {
        return requestOrganization(
                        cloudFoundryClient, ResourceUtils.getEntity(resource).getOrganizationId())
                .map(response -> ResourceUtils.getEntity(response).getName());
    }

    private static Mono<SpaceResource> getOrganizationSpace(
    ...
    private static Mono<String> getOrganizationSpaceId(
            CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
        return getOrganizationSpace(cloudFoundryClient, organizationId, space)
                .map(ResourceUtils::getId);
    }

    private static Mono<String> getOrganizationSpaceIdWhere(
            CloudFoundryClient cloudFoundryClient,
            String organizationId,
            String space,
            Predicate<SpaceResource> predicate) {
        return getOrganizationSpace(cloudFoundryClient, organizationId, space)
                .filter(predicate)
                .map(ResourceUtils::getId);
    }

    private static Mono<List<SecurityGroupEntity>> getSecurityGroups(
            CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource, boolean withRules) {
        return requestSpaceSecurityGroups(cloudFoundryClient, ResourceUtils.getId(spaceResource))
                .map(
                        securityGroupResource -> {
                            SecurityGroupEntity entity =
                                    ResourceUtils.getEntity(securityGroupResource);
                            if (!withRules) {
                                entity =
    ...
    private static Mono<List<String>> getServiceNames(
            CloudFoundryClient cloudFoundryClient, SpaceResource spaceResource) {
    ...
    private static Mono<SpaceDetail> getSpaceDetail(
            CloudFoundryClient cloudFoundryClient,
        ...
        return Mono.zip(
                        ...
                        getSecurityGroups(
                                cloudFoundryClient,
                                resource,
                                Optional.ofNullable(request.getSecurityGroupRules()).orElse(false)),
    ...
    private static Mono<SpaceQuotaDefinitionResource> getSpaceQuota(
            CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
        return requestOrganizationSpaceQuotas(cloudFoundryClient, organizationId, spaceQuota)
                ...
                .onErrorResume(
                        ...
                        t ->
                                ExceptionUtils.illegalArgument(
                                        "Space quota definition %s does not exist", spaceQuota));
    }
    ...
    private static Mono<AssociateSpaceDeveloperByUsernameResponse>
            requestAssociateSpaceDeveloperByUsername(
    ...
    private static Mono<AssociateSpaceManagerByUsernameResponse>
            requestAssociateSpaceManagerByUsername(
                    CloudFoundryClient cloudFoundryClient, String spaceId, String username) {
        return cloudFoundryClient
                .spaces()
    ...
    private static Mono<DeleteSpaceResponse> requestDeleteSpace(
    ...
    private static Mono<GetOrganizationResponse> requestOrganization(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return cloudFoundryClient
                .organizations()
    ...
    private static Flux<SpaceQuotaDefinitionResource> requestOrganizationSpaceQuotas(
            CloudFoundryClient cloudFoundryClient, String organizationId, String spaceQuota) {
        return PaginationUtils.requestClientV2Resources(
                        page ->
                                cloudFoundryClient
    ...
    private static Flux<SpaceResource> requestOrganizationSpaces(
            CloudFoundryClient cloudFoundryClient, String organizationId, String space) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Flux<ApplicationResource> requestSpaceApplications(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Mono<GetSpaceQuotaDefinitionResponse> requestSpaceQuotaDefinition(
    ...
    private static Flux<SecurityGroupResource> requestSpaceSecurityGroups(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Flux<ServiceResource> requestSpaceServices(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Flux<SpaceResource> requestSpaces(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Mono<UpdateSpaceResponse> requestUpdateSpace(
        ...
        return cloudFoundryClient
                .spaces()
                .update(UpdateSpaceRequest.builder().name(newName).spaceId(spaceId).build());
    }

    private static Mono<UpdateSpaceResponse> requestUpdateSpaceSsh(
            CloudFoundryClient cloudFoundryClient, String spaceId, Boolean allowed) {
        return cloudFoundryClient
                .spaces()
                .update(UpdateSpaceRequest.builder().allowSsh(allowed).spaceId(spaceId).build());
    }
    ...
    private static SpaceDetail toSpaceDetail(
    ...
    private static List<SecurityGroup> toSpaceDetailSecurityGroups(
            List<SecurityGroupEntity> securityGroups) {
    ...
    private static SpaceQuota toSpaceQuotaDefinition(
            Resource<SpaceQuotaDefinitionEntity> resource) {
