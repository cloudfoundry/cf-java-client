...
public final class DefaultOrganizations implements Organizations {
    ...
    public DefaultOrganizations(
            Mono<CloudFoundryClient> cloudFoundryClient, Mono<String> username) {
                ...
                .filter(
                        predicate(
                                (cloudFoundryClient,
                                        ...
                                        username) -> setRolesByUsernameEnabled))
                .flatMap(
                        function(
                                (cloudFoundryClient,
                                        organizationId,
    ...
    public Mono<OrganizationDetail> get(OrganizationInfoRequest request) {
        return this.cloudFoundryClient
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, organizationResource) ->
                                        getAuxiliaryContent(
                                                        cloudFoundryClient, organizationResource)
                                                .map(
                                                        function(
    ...
    private static Mono<String> createOrganization(
            CloudFoundryClient cloudFoundryClient, CreateOrganizationRequest request) {
        return Mono.justOrEmpty(request.getQuotaDefinitionName())
                .flatMap(
                        quotaDefinitionName ->
                                getOrganizationQuotaDefinitionId(
                                        cloudFoundryClient, quotaDefinitionName))
                .flatMap(
                        organizationQuotaDefinitionId ->
                                getCreateOrganizationId(
                                        cloudFoundryClient,
                                        request.getOrganizationName(),
                                        organizationQuotaDefinitionId))
                .switchIfEmpty(
                        getCreateOrganizationId(
                                cloudFoundryClient, request.getOrganizationName(), null));
    }

    private static Mono<Void> deleteOrganization(
    ...
    private static Mono<Tuple4<List<String>, OrganizationQuota, List<SpaceQuota>, List<String>>>
            getAuxiliaryContent(
                    CloudFoundryClient cloudFoundryClient,
    ...
    private static Mono<String> getCreateOrganizationId(
    ...
    private static Mono<OrganizationQuota> getOrganizationQuota(
            CloudFoundryClient cloudFoundryClient, OrganizationResource resource) {
        return requestOrganizationQuotaDefinition(
                        cloudFoundryClient,
                        ResourceUtils.getEntity(resource).getQuotaDefinitionId())
                .map(response -> toOrganizationQuota(response, resource));
    }

    private static Mono<OrganizationQuotaDefinitionResource> getOrganizationQuotaDefinition(
        ...
        return requestOrganizationQuotaDefinitions(cloudFoundryClient, quotaDefinitionName)
                ...
                .onErrorResume(
                        ...
                        t ->
                                ExceptionUtils.illegalArgument(
                                        "Organization quota %s does not exist",
                                        quotaDefinitionName));
    }

    private static Mono<String> getOrganizationQuotaDefinitionId(
            CloudFoundryClient cloudFoundryClient, String quotaDefinitionName) {
        return getOrganizationQuotaDefinition(cloudFoundryClient, quotaDefinitionName)
                .map(ResourceUtils::getId);
    }

    private static Mono<List<String>> getSpaceNames(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
    ...
    private static Mono<List<SpaceQuota>> getSpaceQuotas(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
    ...
    private static Mono<AssociateOrganizationManagerByUsernameResponse>
    ...
    private static Mono<CreateOrganizationResponse> requestCreateOrganization(
        ...
        return cloudFoundryClient
                .organizations()
                .create(
                        org.cloudfoundry.client.v2.organizations.CreateOrganizationRequest.builder()
                                .name(organization)
                                .quotaDefinitionId(quotaDefinitionId)
                                .build());
    }

    private static Mono<DeleteOrganizationResponse> requestDeleteOrganization(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return cloudFoundryClient
                .organizations()
                .delete(
                        org.cloudfoundry.client.v2.organizations.DeleteOrganizationRequest.builder()
                                .organizationId(organizationId)
                                .async(true)
                                .build());
    }
    ...
    private static Mono<GetOrganizationQuotaDefinitionResponse> requestOrganizationQuotaDefinition(
    ...
    private static Flux<OrganizationQuotaDefinitionResource> requestOrganizationQuotaDefinitions(
            CloudFoundryClient cloudFoundryClient, String organizationQuotaDefinition) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
                                .organizationQuotaDefinitions()
    ...
    private static Flux<SpaceQuotaDefinitionResource> requestSpaceQuotaDefinitions(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
        return PaginationUtils.requestClientV2Resources(
                page ->
                        cloudFoundryClient
    ...
    private static Mono<UpdateOrganizationResponse> requestUpdateOrganization(
            CloudFoundryClient cloudFoundryClient, String organizationId, String newName) {
        return cloudFoundryClient
                .organizations()
                .update(
    ...
    private static Mono<Void> setOrganizationManager(
            CloudFoundryClient cloudFoundryClient, String organizationId, String username) {
        return Mono.zip(
                        requestAssociateOrganizationManagerByUsername(
                                cloudFoundryClient, organizationId, username),
    ...
    private static OrganizationDetail toOrganizationDetail(
            List<String> domains,
            OrganizationQuota organizationQuota,
            List<SpaceQuota> spacesQuotas,
            List<String> spaces,
            OrganizationResource organizationResource,
    ...
    private static OrganizationQuota toOrganizationQuota(
            GetOrganizationQuotaDefinitionResponse response, OrganizationResource resource) {
