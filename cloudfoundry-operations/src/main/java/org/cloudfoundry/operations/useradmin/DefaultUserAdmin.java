...
public final class DefaultUserAdmin implements UserAdmin {
    ...
    public DefaultUserAdmin(
            Mono<CloudFoundryClient> cloudFoundryClient, Mono<UaaClient> uaaClient) {
    ...
    public Mono<Void> delete(DeleteUserRequest request) {
        return Mono.zip(this.cloudFoundryClient, this.uaaClient)
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, uaaClient, userId) ->
                                        Mono.zip(
                ...
                .flatMap(
                        cloudFoundryClient ->
                                Mono.zip(
                                        Mono.just(cloudFoundryClient),
                                        getOrganizationId(
                                                cloudFoundryClient, request.getOrganizationName())))
                .flatMap(
                        function(
                                (cloudFoundryClient, organizationId) ->
                                        Mono.zip(
                                                listOrganizationAuditorNames(
                                                        cloudFoundryClient, organizationId),
                                                listOrganizationBillingManagerNames(
                                                        cloudFoundryClient, organizationId),
                                                listOrganizationManagerNames(
                                                        cloudFoundryClient, organizationId))))
                        ...
                        function(
                                (cloudFoundryClient, spaceId) ->
                                        Mono.zip(
                                                ...
                                                listSpaceDeveloperNames(
                                                        cloudFoundryClient, spaceId),
                                                listSpaceManagerNames(
                                                        cloudFoundryClient, spaceId))))
        ...
        return this.cloudFoundryClient
                        ...
                        cloudFoundryClient ->
                                Mono.zip(
                                        ...
                                        getFeatureFlagEnabled(
                                                cloudFoundryClient,
                                                SET_ROLES_BY_USERNAME_FEATURE_FLAG)))
                .filter(
                        predicate(
                                (cloudFoundryClient, setRolesByUsernameEnabled) ->
                                        setRolesByUsernameEnabled))
                .switchIfEmpty(
                        ExceptionUtils.illegalState("Setting roles by username is not enabled"))
                .flatMap(
                        function(
                                (cloudFoundryClient, ignore) ->
                                        Mono.zip(
                                                Mono.just(cloudFoundryClient),
                                                getOrganizationId(
                                                        cloudFoundryClient,
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, organizationId) ->
                                        Mono.zip(
                                                requestAssociateOrganizationUserByUsername(
                                                        cloudFoundryClient,
                                                        organizationId,
                                                        request),
                                                associateOrganizationRole(
                                                        cloudFoundryClient,
                                                        organizationId,
                                                        request))))
    ...
    public Mono<Void> setSpaceRole(SetSpaceRoleRequest request) {
        return this.cloudFoundryClient
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId, ignore) ->
                                        associateSpaceRole(cloudFoundryClient, request, spaceId)))
                ...
                .filter(
                        predicate(
                                (cloudFoundryClient, setRolesByUsernameEnabled) ->
                                        setRolesByUsernameEnabled))
                .switchIfEmpty(
                        ExceptionUtils.illegalState("Unsetting roles by username is not enabled"))
                .flatMap(
                        function(
                                (cloudFoundryClient, ignore) ->
                                        Mono.zip(
                        ...
                        function(
                                (cloudFoundryClient, organizationId) ->
                                        removeOrganizationRole(
                                                cloudFoundryClient, organizationId, request)))
    ...
    public Mono<Void> unsetSpaceRole(UnsetSpaceRoleRequest request) {
        return this.cloudFoundryClient
                ...
                .flatMap(
                        function(
                                (cloudFoundryClient, spaceId) ->
                                        removeSpaceRole(cloudFoundryClient, request, spaceId)))
                .transform(OperationsLogging.log("Unset User Space Role"))
                .then();
    }

    private static Mono<Void> associateOrganizationRole(
    ...
    private static Mono<AssociateOrganizationUserByUsernameResponse> associateOrganizationRole(
            CloudFoundryClient cloudFoundryClient, String username, String organizationId) {
        return cloudFoundryClient
                .organizations()
    ...
    private static Mono<Void> associateSpaceRole(
            CloudFoundryClient cloudFoundryClient, SetSpaceRoleRequest request, String spaceId) {
    ...
    private static Mono<Boolean> getFeatureFlagEnabled(
            CloudFoundryClient cloudFoundryClient, String featureFlag) {
    ...
    private static Mono<List<String>> listOrganizationAuditorNames(
    ...
    private static Mono<List<String>> listOrganizationBillingManagerNames(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
    ...
    private static Mono<List<String>> listOrganizationManagerNames(
            CloudFoundryClient cloudFoundryClient, String organizationId) {
    ...
    private static Mono<List<String>> listSpaceAuditorNames(
    ...
    private static Mono<List<String>> listSpaceDeveloperNames(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
    ...
    private static Mono<List<String>> listSpaceManagerNames(
            CloudFoundryClient cloudFoundryClient, String spaceId) {
    ...
    private static Mono<Void> removeOrganizationRole(
            CloudFoundryClient cloudFoundryClient,
            String organizationId,
            UnsetOrganizationRoleRequest request) {
        if (OrganizationRole.AUDITOR == request.getOrganizationRole()) {
            return cloudFoundryClient
                    .organizations()
    ...
    private static Mono<Void> removeSpaceRole(
            CloudFoundryClient cloudFoundryClient, UnsetSpaceRoleRequest request, String spaceId) {
        if (SpaceRole.AUDITOR == request.getSpaceRole()) {
            return cloudFoundryClient
                    .spaces()
                    .removeAuditorByUsername(
    ...
    private static Mono<AssociateOrganizationUserByUsernameResponse>
            requestAssociateOrganizationUserByUsername(
                    CloudFoundryClient cloudFoundryClient,
                    String organizationId,
                    SetOrganizationRoleRequest request) {
    ...
    private static Mono<CreateUserResponse> requestCreateUaaUser(
            UaaClient uaaClient, CreateUserRequest request) {
        return uaaClient
                .users()
                .create(
    ...
    private static Mono<org.cloudfoundry.client.v2.users.CreateUserResponse> requestCreateUser(
            CloudFoundryClient cloudFoundryClient, String userId) {
        return cloudFoundryClient
                .users()
                .create(
                        org.cloudfoundry.client.v2.users.CreateUserRequest.builder()
                                .uaaId(userId)
                                .build());
    }

    private static Mono<DeleteUserResponse> requestDeleteUaaUser(
            UaaClient uaaClient, String userId) {
        return uaaClient
                .users()
                .delete(
                        org.cloudfoundry.uaa.users.DeleteUserRequest.builder()
                                .userId(userId)
                                .build());
    }

    private static Mono<org.cloudfoundry.client.v2.users.DeleteUserResponse> requestDeleteUser(
            CloudFoundryClient cloudFoundryClient, String userId) {
        return cloudFoundryClient
                .users()
                .delete(
                        org.cloudfoundry.client.v2.users.DeleteUserRequest.builder()
                                .async(true)
                                .userId(userId)
                                .build());
    }

    private static Mono<GetFeatureFlagResponse> requestGetFeatureFlag(
            CloudFoundryClient cloudFoundryClient, String featureFlag) {
        return cloudFoundryClient
                .featureFlags()
                .get(GetFeatureFlagRequest.builder().name(featureFlag).build());
    }
    ...
    private Mono<OrganizationUsers> toOrganizationUsers(
            List<String> auditors, List<String> billingManagers, List<String> managers) {
        return Mono.just(
    ...
    private Mono<SpaceUsers> toSpaceUsers(
            List<String> auditors, List<String> developers, List<String> managers) {
        return Mono.just(
                SpaceUsers.builder()
                        .addAllAuditors(auditors)
