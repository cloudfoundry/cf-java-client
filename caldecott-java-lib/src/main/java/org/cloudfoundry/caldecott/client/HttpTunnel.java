...
public class HttpTunnel implements Tunnel {
	...
	private void openTunnel() {
		...
		HttpEntity<String> response = restOperations.exchange(url + "/tunnels", HttpMethod.POST, requestEntity, String.class);
		String jsonResponse = response.getBody().trim();
		try {
			this.tunnelInfo = new ObjectMapper().readValue(jsonResponse, TypeFactory.mapType(HashMap.class, String.class, String.class));
	...
	public void close() {
		...
		try {
			restOperations.exchange(url + this.tunnelInfo.get("path"), HttpMethod.DELETE, requestEntity, String.class);
	...
	private void sendBytes(byte[] bytes, long page) {
		...
		ResponseEntity<?> response = restOperations.exchange(dataUrl, HttpMethod.PUT, requestEntity, String.class);
		if (logger.isDebugEnabled())
