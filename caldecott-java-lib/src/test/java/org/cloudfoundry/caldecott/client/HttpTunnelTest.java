...
public class HttpTunnelTest {
	...
	public void testOpenTunnel() {
		openTunnel();
		verify(restTemplate, times(1)).exchange(isA(String.class), isA(HttpMethod.class), isA(HttpEntity.class), isA(Class.class));
	}
	...
	public void testClosingTunnel() {
		openTunnel();
		httpTunnel.close();
		verify(restTemplate, times(2)).exchange(isA(String.class), isA(HttpMethod.class), isA(HttpEntity.class), isA(Class.class));
	}
	...
	public void testWritingSomeBytes() {
		...
		when(restTemplate.exchange(isA(String.class), isA(HttpMethod.class), isA(HttpEntity.class), isA(Class.class)))
				.thenAnswer(new Answer<ResponseEntity<?>>() {
					public ResponseEntity<?> answer(InvocationOnMock invocation) throws Throwable {
						...
						System.out.println("+++ " + arg3.getBody());
	...
	public void testReadingSomeBytes() {
		...
		openTunnel();
		when(restTemplate.execute(isA(String.class), isA(HttpMethod.class), isA(RequestCallback.class), isA(ResponseExtractor.class)))
				.thenReturn(data);
		final byte[] answer = httpTunnel.read(false);
		verify(restTemplate, times(1)).execute(isA(String.class), isA(HttpMethod.class), isA(RequestCallback.class), isA(ResponseExtractor.class));
